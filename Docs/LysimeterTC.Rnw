...
\begin{figure}[!h]\label{figure:temp_LysimeterTotal C}
...
<<TableTempTC, echo=FALSE, results='asis'>>=
...
<<StatTempTotalCS, echo=TRUE, results='markup'>>=
...
\begin{figure}[!h]\label{figure:Ch_LysimeterTotal C}
